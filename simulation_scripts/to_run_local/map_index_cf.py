...
def main():
    job_id=sys.argv[1]
    var_dict={
    #'K': [5e-5,5.5e-5,6e-5,6.5e-5,7e-5,7.5e-5,8e-5,8.5e-5,9e-5,9.5e-5,1e-4], #np.geomspace(1,20,8)*1e-4,
    'K': [8e-5], #np.geomspace(1,20,8)*1e-4,
    #'R_vert_frac': [0.4],
    #'noise_iterations':[1000, 10000, 100000, 1000000],
    'seed': np.arange(1),#np.arange(1), #np.arange(5),
    #'int_curvature_MD':[0,],#[ -10, -15 ], #here we give negative value because the orientation of the curve is clockwise in the code
    'int_curvature_CF':[20,],
    #'germ_band_push' : [0,0.05,0.1,0.15,0.2,0.25,0.3,0.35], #np.linspace(0,0.35, 8), 
    'germ_band_push' : [0.35], #np.linspace(0,0.35, 8), 
    #'noise_scale' : [1e-5],
    #'dt' : [1e-5],
