...
def load_timepoints(glob_path, last_timepoint_only = False, npart = 120):
    ...
    for i, path in enumerate(glob.glob(glob_path)):
        ...
        try: 
            ...
            initial_df = pd.read_csv(path + '/reference_balls.csv')
        except: 
...
#methods that I use during postprocess

def combine_files_from_folders(glob_path = '*/', filename = 'summed_prop_per_timepoint.csv', verbose = True):
...
def load_timepoints(glob_path, last_timepoint_only = True, reference_file = 'reference_balls.csv'):
    ...
    for i, path in enumerate(glob.glob(glob_path)):
        ...
        try: 
            ...
            try:
                df_allTimes = pd.read_csv(path + '/database.csv')
            except:
                print('files corrupted or empty path: ' + path)
                continue
            t = max(df_allTimes["t"].values)
            df_temp = df_allTimes.query("t == @t")
            df_temp.to_csv(path + '/final.csv', index = False)
                
        try:
            initial_df = pd.read_csv(path +'/'+reference_file)
        except:
            t_init = min(df_allTimes["t"].values)
            initial_df = df_allTimes.query("t == @t_init")
            print("no initial file: " + path)

            
        #initial_df_common = initial_df[["ID", "mitotic_domain", "mitotic_domain_id","dr0", "K", "L+1", "L-1","ko","arclength", "arclength_frac"]]
        #df_temp = pd.concat([initial_df_common, df_temp], axis = 1)
        cols_from_init = ["ID", "mitotic_domain", "mitotic_domain_id","dr0", "K", "L+1", "L-1","ko","arclength", "arclength_frac"]
        for col in cols_from_init: 
            if col not in df_temp.columns: df_temp[col] = initial_df[col]
        df_temp["init_x"] = initial_df["x"]
        df_temp["init_y"] = initial_df["y"]
