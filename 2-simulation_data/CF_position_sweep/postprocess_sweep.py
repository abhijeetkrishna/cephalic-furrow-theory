...
groupby_cols = ["K", "germ_band_push", 'int_curvature_CF', 'CF_position', 't_MD']
path_to_dfs = "dfs/"
...
def get_fold_analysis(row, t = None, colName_suffix = "", fold_threshold = 0.035):
    ...
    if (t is None) and (colName_suffix == "_at_maxBendEnergy_afterMD" ):
        ...
        t_MD = row["t_MD"] if "t_MD" in row.index else 0
...
grouped_df = grouped_df.apply(get_fold_analysis, t = None, colName_suffix = "_at_maxBendEnergy_afterMD", fold_threshold = fold_threshold, axis = 1)
...
df_allSims_at_maxBendEnegy = combine_files_from_folders(glob_path = "*/", filename="balls_w_fold_analysis_at_maxBendEnergy_afterMD.csv", verbose = False)
...
df_mean_shape_at_maxBendEnegy.to_csv(path_to_dfs + "mean_depth_along_curve_at_maxBendEnergy_afterMD.csv", index = False)
